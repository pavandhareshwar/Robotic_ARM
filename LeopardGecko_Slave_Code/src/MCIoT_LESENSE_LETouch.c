...
/**************************************************************************//**
 * @brief Initializes the capacitive touch system with LESENSE.
 *
...
static void LETOUCH_setupLESENSE( void )
{
	...
	CMU_ClockSelectSet(cmuClock_LFA, cmuOsc_LFXO);
	CMU_ClockEnable(cmuClock_LESENSE, true);

	/* LESENSE configuration structure */
	static const LESENSE_Init_TypeDef lesense_init =
	{
		.coreCtrl =
		{
			.scanStart		= LESENSE_CORE_CTRL_SCAN_MODE,
			.prsSel			= LESENSE_CORE_CTRL_PRS_SOURCE,
			.scanConfSel	= LESENSE_CORE_CTRL_SCAN_CONFIG,
			.invACMP0		= false,
			.invACMP1		= false,
			.dualSample		= false,
			.storeScanRes 	= false,
			.bufOverWr		= true,
			.bufTrigLevel 	= LESENSE_CORE_CTRL_BUFTRIG_LEVEL,
			.wakeupOnDMA	= LESENSE_CORE_CTRL_DMA_WAKEUP,
			.biasMode		= LESENSE_CORE_CTRL_BIAS_MODE,
			.debugRun		= false
		},

		.timeCtrl =
		{
			.startDelay		= LESENSE_TIM_CTRL_START_DELAY
		},

		.perCtrl =
		{
			.dacCh0Data		= LESENSE_PER_CTRL_DAC_DATAOUT,
			.dacCh0ConvMode	= LESENSE_PER_CTRL_DAC_CONV_MODE,
			.dacCh0OutMode	= LESENSE_PER_CTRL_DAC_OUT_MODE,
			.dacCh1Data		= LESENSE_PER_CTRL_DAC_DATAOUT,
			.dacCh1ConvMode = LESENSE_PER_CTRL_DAC_CONV_MODE,
			.dacCh1OutMode	= LESENSE_PER_CTRL_DAC_OUT_MODE,
			.dacPresc		= 0,
			.dacRef			= LESENSE_PER_CTRL_DAC_REF,
			.acmp0Mode		= LESENSE_PER_CTRL_ACMP_MODE,
			.acmp1Mode		= LESENSE_PER_CTRL_ACMP_MODE,
			.warmupMode		= LESENSE_PER_CTRL_WARMUP_MODE
		},

		.decCtrl =
		{
			.decInput		= LESENSE_DEC_CTRL_DEC_INPUT,
			.initState		= 0,
			.chkState		= false,
			.intMap			= false,
			.hystPRS0		= false,
			...
			.hystIRQ 		= false,
			.prsCount		= false,
			.prsChSel0 		= LESENSE_DEC_CTRL_PRS_CH0_SRC,
			.prsChSel1 		= LESENSE_DEC_CTRL_PRS_CH1_SRC,
			.prsChSel2 		= LESENSE_DEC_CTRL_PRS_CH2_SRC,
			.prsChSel3 		= LESENSE_DEC_CTRL_PRS_CH3_SRC
		}
	};
	...
	static const LESENSE_ChDesc_TypeDef initLesenseCh =
	{
		...
		.enaPin			= true,
		.enaInt			= true,
		.chPinExMode	= LESENSE_CH_DESC_CH_PIN_MODE_EXCITE_PHASE,
		.chPinIdleMode	= LESENSE_CH_DESC_CH_PIN_MODE_IDLE_PHASE,
		.useAltEx		= false,
		.shiftRes		= false,
		.invRes			= false,
		.storeCntRes	= true,
		.exClk			= LESENSE_CH_DESC_EXC_SAMPLE_CLK,
		.sampleClk		= LESENSE_CH_DESC_EXC_SAMPLE_CLK,
		.exTime			= LESENSE_CH_DESC_EXCITE_TIME,
		.sampleDelay	= SAMPLE_DELAY,
		.measDelay		= LESENSE_CH_DESC_MEASURE_DELAY,
		.acmpThres		= LESENSE_CH_DESC_ACMP_THRES,
		.sampleMode		= LESENSE_CH_DESC_SAMPLE_MODE,
		.intMode		= LESENSE_CH_DESC_INT_MODE,
		.cntThres		= LESENSE_CH_DESC_DATA_COMP_DEC_THRESH,
		.compMode		= LESENSE_CH_DESC_CH_COMP_MODE
	};

	/* Initialize LESENSE interface _with_ RESET. */
	LESENSE_Init(&lesense_init, true);
